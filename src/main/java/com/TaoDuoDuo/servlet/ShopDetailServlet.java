...
public class ShopDetailServlet extends HttpServlet {
        ...
        try {
            ...
            if ("addProduct".equals(action)) {
                ...
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=invalid_action");
            }
            ...
            e.printStackTrace();
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopIdStr + "&error=system_error");
        }
    }
    ...
    private void handleAddProduct(HttpServletRequest request, HttpServletResponse response, int shopId)
        ...
        if (productName == null || productName.trim().isEmpty() ||
            ...
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=invalid_params");
            return;
        }
        ...
        try {
            ...
            // 如果没有选择分类，使用默认分类8
            int categoryId = 8; // 默认分类
            if (categoryIdStr != null && !categoryIdStr.trim().isEmpty()) {
                try {
                    categoryId = Integer.parseInt(categoryIdStr);
                } catch (NumberFormatException e) {
                    categoryId = 8; // 解析失败时使用默认分类
                }
            }

            if (price < 0 || stock < 0 || categoryId <= 0) {
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=invalid_values");
                return;
            }

            // 创建商品（使用用户选择的分类ID或默认分类8）
            Product product = new Product(productName.trim(), description.trim(), price, stock, categoryId, shopId);
            ...
            if (success) {
                ...
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=add_failed");
            }

        } catch (NumberFormatException e) {
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=invalid_numbers");
        }
    }
    ...
    private void handleUpdateShop(HttpServletRequest request, HttpServletResponse response, int shopId)
        ...
        if (newShopName.length() < 2 || newShopName.length() > 50) {
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=name_length");
            return;
        }
        ...
        if (success) {
            ...
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=name_exists");
        }
    }
    ...
    private void handleUpdateProduct(HttpServletRequest request, HttpServletResponse response, int shopId)
        ...
        try {
            ...
            if (product == null || product.getShop_id() != shopId) {
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=product_not_found");
                return;
            }
            ...
            if (success) {
                ...
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=update_failed");
            }
    ...
    private void handleDeleteProduct(HttpServletRequest request, HttpServletResponse response, int shopId)
        ...
        try {
            ...
            if (success) {
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&success=product_deleted");
            } else {
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=delete_failed");
            }

        } catch (NumberFormatException e) {
            response.sendRedirect(
                    request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=invalid_product_id");
        }
    }
    ...
    private void handleToggleListing(HttpServletRequest request, HttpServletResponse response, int shopId)
        ...
        try {
            ...
            if (success) {
                ...
                response.sendRedirect(
                        request.getContextPath() + "/ShopDetailServlet?shopId=" + shopId + "&error=listing_failed");
            }
