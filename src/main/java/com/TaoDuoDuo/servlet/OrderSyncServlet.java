...
public class OrderSyncServlet extends HttpServlet {
        ...
        try {
            ...
            if (queryResponse != null) {
                ...
                if (needUpdate) {
                    ...
                    if (updated) {
                        out.println("<p style='color: green;'>✅ 订单状态已更新: " + order.getOrder_status() + " → " + newStatus + "</p>");
                    } else {
    ...
    private void showUnpaidOrders(PrintWriter out) {
        try {
            ...
            if (userId == null) {
                // 从session获取
                userId = 1; // 临时使用用户ID 1，实际应该从session获取
            }
            ...
            if (ordersOpt.isPresent() && !ordersOpt.get().isEmpty()) {
                ...
                for (Order order : ordersOpt.get()) {
                    ...
                    out.println("<td>" + (order.getAlipay_order_no() != null ? order.getAlipay_order_no() : "无") + "</td>");
