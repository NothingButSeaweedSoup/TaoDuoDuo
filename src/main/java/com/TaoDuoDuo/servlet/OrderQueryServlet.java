            ...
            if (userRole == 1) { // 普通用户
                ...
                if (userShops != null && !userShops.isEmpty()) {
                    for (Shop shop : userShops) {
                        Optional<List<Order>> shopOrdersOpt;
                        if (statusFilter != null && !statusFilter.isEmpty()) {
                            shopOrdersOpt = orderDao.getOrderByShopId(shop.getShop_id(), statusFilter);
                        } else {
                            shopOrdersOpt = orderDao.getOrderByShopId(shop.getShop_id());
                        }

                        if (shopOrdersOpt.isPresent()) {
                            allOrders.addAll(shopOrdersOpt.get());
                        }
                    }
                }

                ordersOpt = Optional.of(allOrders);
